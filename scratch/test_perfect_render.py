...
print("Setup complete for perfect render.")

def generate_figure(out_path):
    ...
    add_region_label(ax1, fig, 2.05, 0.42, "minus", "#1b4f72", fontsize=33)
    add_region_label(ax1, fig, 1.45, 3.10, "plus", "#922b21", fontsize=33)
    ...
    ax1.text(V0a + 0.10, 0.5 * (U0a + U_GT_a), r"$\mathbf{V(X_>) = V_{X_0} \in \rho(A_X)}$" "\n" r"$\mathbf{(Heating\ Slice)}$",
    ...
    add_curved_callout(ax1, fig, r"$\mathbf{\partial A_{X_1}}$", (1.65, float(U_adiabat(1.65, S1))), (1.80, 3.65), box_curve_adia, color=C_ADIA, fontsize=18.0, lw=1.5, rad=-0.12, shrinkB=0)
    ...
    add_curved_callout(ax1, fig, r"$\mathbf{X_0}$", (V0a, U0a), (1.18, 0.55), box_white, color="#4e2103", fontsize=17.5, lw=1.6, rad=-0.14, preferred_corner="tl", shrinkB=8)
    ...
    add_curved_callout(ax1, fig, r"$\mathbf{X_>}$", (V0a, U_GT_a), (0.95, 3.45), box_gt, color="#784212", fontsize=17.5, lw=1.6, rad=-0.14, preferred_corner="bl", shrinkB=8)
    add_curved_callout(ax1, fig, r"$\mathbf{X^\prime \overset{A}{\sim} X}$", (V_PRIME, U_PRIME), (1.95, 1.25), box_cut, color=C_PT_CUT, fontsize=17.5, lw=1.8, rad=-0.14, preferred_corner="tl", shrinkB=10)
    ...
    add_curved_callout(ax2, fig, r"$\mathbf{exit\ of\ \partial A_X\ through\ \partial\Gamma\ (limit,\ not\ a\ state)}$",
                       (V_B, U_EXIT), (1.45, 3.45), box_white, color=C_GAMMA, fontsize=12.5, lw=1.5, rad=0.10, preferred_corner="bl", shrinkB=6)

    # dA_X1 ends on ceiling
    ...
    # Smooth curve inside Omega_> from X'_0 to X'_1 crossing dA_X at X_up
    pts_path_v = np.array([V0b, 1.05, V_UP, 1.95, V1])
    pts_path_u = np.array([U0_prime, 2.70, U_UP, 2.85, U1_prime])
    ...
    add_region_label(ax2, fig, 1.45, 3.10, "plus", "#922b21", fontsize=33)
    ...
    add_curved_callout(ax2, fig, r"$\mathbf{X^\prime_0 \in \Omega_>\ (X^\prime_0 \prec\prec X)}$" "\n" r"$\mathbf{(Axiom\ T5:\ full\ temperature\ range\ on\ every\ column\ \Rightarrow\ hot\ state\ below\ the\ adiabat\ level)}$",
                       (V0b, U0_prime), (1.10, 1.45), box_prime0, color="#922b21", fontsize=11.5, lw=1.6, rad=-0.14, preferred_corner="tl", shrinkB=8)
    ...
    add_curved_callout(ax2, fig, r"$\mathbf{X_\uparrow \in \partial A_X \cap \Omega_>}$", (V_UP, U_UP), (1.20, 2.95), box_gt, color="#784212", fontsize=15.0, lw=1.6, rad=-0.12, preferred_corner="bl", shrinkB=8)
...
generate_figure("scratch/perfect_scenario1.png")
