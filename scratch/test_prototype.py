...
def build_stacked_figure(out_path):
    # Height distribution:
    # row 0: shared legend (0.35)
    # row 1: Case (a) (5.0)
    # row 2: Case (b) (5.0)
    # row 3: Proof banner (1.85)
    fig = plt.figure(figsize=(18, 25.5), dpi=300)
    gs = gridspec.GridSpec(4, 1, height_ratios=[0.38, 5.0, 5.0, 1.85], hspace=0.26)
    ...
    leg = ax_legend.legend(handles=legend_elements, labels=legend_labels,
                          ...
                          loc="center", ncol=4, fontsize=16.0,
                          handlelength=3.4, handleheight=1.2, columnspacing=3.2,
    ...
    # Regions
    add_region_label(ax1, fig, 2.05, 1.85, "minus", "#1b4f72", fontsize=33)
    ...
    ax2.plot([u_min, u_max], [V0, V0], color="#784212", ls="--", lw=1.8, zorder=4)

    # Sub-step (b): Hot state X'_0 on ell (T5 + T0 < Tmax gives T(X'_0) > T0, yet X'_0 << X)
    U0_PRIME = 3.35
    T0_PRIME = temperature(U0_PRIME, V0)
    add_dotted_arrow(ax2, (U0, V0), (U0_PRIME, V0), color="#c0392b", lw=4.2, mutation_scale=32,
                     head_length=0.82, head_width=0.35, shrinkA=9.0, shrinkB=12.0, dot_spacing=2.0, zorder=9)

    box_ell = dict(boxstyle="round,pad=0.18", facecolor="#fffcfc", edgecolor="#c0392b", lw=1.5, alpha=1.0)
    ax2.text(0.5 * (U0 + U0_PRIME), V0 + 0.10,
             r"$\mathbf{Axiom\ T5:\ X^\prime_0 \in \Omega_>\ (T > T_0)\ with\ X^\prime_0 \prec\prec X\ (\ell \cap \partial A_X = \emptyset)}$",
             fontsize=14.5, fontweight="bold", color="#922b21", ha="center", va="center", zorder=100, bbox=box_ell)
    ...
    add_dotted_arrow(ax2, (U1, V1), (U1_PRIME, V1), color="#c0392b", lw=4.2, mutation_scale=32,
                     head_length=0.82, head_width=0.35, shrinkA=9.0, shrinkB=12.0, dot_spacing=2.0, zorder=9)
    box_ell1 = dict(boxstyle="round,pad=0.18", facecolor="#fffcfc", edgecolor="#c0392b", lw=1.5, alpha=1.0)
    ax2.text(0.5 * (U1 + U1_PRIME), V1 + 0.10,
             r"$\mathbf{Planck\ and\ T5:\ X^\prime_1 \in \Omega_>\ (T > T_0)\ with\ X \prec\prec X_1 \prec X^\prime_1}$",
             fontsize=14.5, fontweight="bold", color="#922b21", ha="center", va="center", zorder=100, bbox=box_ell1)
    ...
    add_region_label(ax2, fig, 2.05, 1.85, "minus", "#1b4f72", fontsize=33)
    add_region_label(ax2, fig, 4.35, 1.95, "plus", "#922b21", fontsize=33)

    # Indicate rho(A_X) range on vertical axis with thick vertical bar
    ax2.plot([u_min + 0.02, u_min + 0.02], [v_rho_min, v_max], color="#2c3e50", lw=6.0, zorder=10)
    ax2.text(u_min + 0.06, 0.5 * (v_rho_min + v_max), r"$\mathbf{\rho(A_X)\ [Domain\ of\ \partial A_X]}$",
             fontsize=13.5, fontweight="bold", color="#2c3e50", rotation=90, va="center", ha="left", zorder=100)
    ax2.text(u_min + 0.06, V0, r"$\mathbf{V_{X_0} \notin \rho(A_X)}$",
             fontsize=13.5, fontweight="bold", color="#c0392b", va="center", ha="left", zorder=100)
    ...
    ax2.set_yticklabels([r"$\mathbf{V_{X_0}}$", r"$\mathbf{\inf \rho(A_X)}$", r"$\mathbf{V_{X_1}}$"], fontsize=14, fontweight="bold")

    # Callouts for Case (b)
    add_curved_callout(ax2, fig, r"$\mathbf{X_0}$", (U0, V0), (1.96, 0.98), box_white, color="#4e2103", fontsize=17.5, lw=1.6, rad=-0.14, preferred_corner="br", shrinkB=8)
    add_curved_callout(ax2, fig, r"$\mathbf{X_1}$", (U1, V1), (2.42, 2.58), box_white, color="#4e2103", fontsize=17.5, lw=1.6, rad=-0.12, preferred_corner="br", shrinkB=8)
    add_curved_callout(ax2, fig, r"$\mathbf{X^\prime_0}$", (U0_PRIME, V0), (3.88, 0.95), box_prime0, color="#922b21", fontsize=17.5, lw=1.6, rad=-0.14, preferred_corner="bl", shrinkB=8)
    add_curved_callout(ax2, fig, r"$\mathbf{X^\prime_1}$", (U1_PRIME, V1), (4.15, 2.58), box_prime1, color="#922b21", fontsize=17.5, lw=1.6, rad=0.14, preferred_corner="tl", shrinkB=8)
    add_curved_callout(ax2, fig, r"$\mathbf{X_\uparrow \in \partial A_X \cap \Omega_>}$", (U_UP, V_UP), (3.48, 1.30), box_gt, color="#784212", fontsize=16.5, lw=1.6, rad=-0.12, preferred_corner="bl", shrinkB=8)
    add_curved_callout(ax2, fig, r"$\mathbf{X_\downarrow \in \partial A_X \cap \Omega_<}$", (U_DOWN, V_DOWN), (1.30, 2.45), box_lt, color="#1b4f72", fontsize=16.5, lw=1.6, rad=0.12, preferred_corner="tr", shrinkB=8)
    ...
    fig.suptitle("Theorem 5.5 — Step 2 (Scenario 1): Interior Transversal Crossing\n"
                 ...
                 fontsize=22.0, fontweight="bold", color="#1a252f", y=0.992)
