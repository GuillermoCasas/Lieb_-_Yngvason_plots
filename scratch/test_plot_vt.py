...
Vt_val = 1.35        # generic point V(t) at 1.35, which is also the endpoint of the shaded segment
...
# Integrate the adiabats for the distance t in [0, Vt_val - V0]
Vgrid = np.linspace(V0, Vt_val, 400)
sol0 = solve_ivp(adiabat_rhs, (V0, Vt_val), [U0], t_eval=Vgrid, rtol=1e-10, atol=1e-12)
sole = solve_ivp(adiabat_rhs, (V0, Vt_val), [U0 + EPS], t_eval=Vgrid, rtol=1e-10, atol=1e-12)
...
# Lipschitz constant is evaluated at V0
C_LIP = (GAMMA - 1.0) / V0
...
# Here we keep a ball radius of 0.5 (so 2\tau = 1.0) to look visually perfect in the grid
...
# Point on lower adiabat at generic point Vt_val
...
ax1.annotate("", xy=(Vt_val, 1.0), xytext=(Vt_val, W0_Vt),
             arrowprops=dict(arrowstyle="->", color="purple", lw=1.5, ls=":", shrinkA=0, shrinkB=0),
             zorder=4)
ax1.text(Vt_val + 0.03, (W0_Vt + 1.0) / 2, r"$\Pi(U, t)$", color="purple", fontsize=11, va="center")
...
ax1.plot([Vt_val], [1.0], "x", color="purple", ms=6, mew=1.5, zorder=5, clip_on=False)
...
ax1.plot([0.0, Vt_val], [W0_Vt, W0_Vt], color='gray', ls=':', lw=1.0)
ax1.plot([0.0, Vt_val], [We_Vt, We_Vt], color='gray', ls=':', lw=1.0)

# Vertical dashed lines from V0 and V(t) on the x-axis to the curves
ax1.plot([V0, V0], [1.0, We_V0], color='gray', ls=':', lw=1.0)
ax1.plot([Vt_val, Vt_val], [1.0, We_Vt], color='gray', ls=':', lw=1.0)
...
ax1.plot([0.0], [W0_Vt], "o", color="black", ms=4, zorder=5, clip_on=False)
ax1.plot([0.0], [We_Vt], "o", color="black", ms=4, zorder=5, clip_on=False)
...
fig.savefig("scratch/test_lemma51_vt.png", dpi=150, bbox_inches="tight")
print("Saved test figure Vt.")
