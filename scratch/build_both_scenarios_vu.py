...
def generate_scenario1(out_path):
    ...
    ax1.contourf(V_2D, U_2D, T_2D, levels=80, cmap="coolwarm", norm=norm, alpha=0.22, zorder=1)

    U_GT = float(U_adiabat(V0, S_MID))
    U_LT = float(U_adiabat(V1, S_MID))

    mask0 = (u_adia0_vals >= u_min) & (u_adia0_vals <= u_max)
    ax1.plot(v_grid[mask0], u_adia0_vals[mask0], color=C_ADIA, lw=2.4, ls="-", zorder=5)
    mask1 = (u_adia1_vals >= u_min) & (u_adia1_vals <= u_max)
    ax1.plot(v_grid[mask1], u_adia1_vals[mask1], color=C_ADIA, lw=2.4, ls="-", zorder=5)
    maskX = (u_adiaX_vals >= u_min) & (u_adiaX_vals <= u_max)
    ax1.plot(v_grid[maskX], u_adiaX_vals[maskX], color=C_ADIA, lw=3.8, ls="-", zorder=6)
    mask_iso = (u_iso_vals >= u_min) & (u_iso_vals <= u_max)
    ax1.plot(v_grid[mask_iso], u_iso_vals[mask_iso], color=C_ISO, lw=3.6, ls=(0, (6, 3.5)), zorder=7)

    # Points in Panel 1: (V, U)
    ax1.scatter([V0], [U0], color=C_PT_ISO, s=190, zorder=15, edgecolors="#4e2103", lw=2.2)
    ax1.scatter([V1], [U1], color=C_PT_ISO, s=190, zorder=15, edgecolors="#4e2103", lw=2.2)
    ax1.scatter([V0], [U_GT], color=C_PT_GT, s=210, zorder=15, edgecolors="#6e4402", lw=2.2)
    ...
    # X_- is below I_{T_0} (cooler), X_+ is above I_{T_0} (warmer)
    add_region_label(ax1, fig, 2.30, 0.45, "minus", "#1b4f72", fontsize=33)
    add_region_label(ax1, fig, 1.45, 3.25, "plus", "#922b21", fontsize=33)

    # Proof Arrows: Vertical Heating and Cooling Slices at constant V
    # Heating: V = V0, U0 -> U_GT (upwards)
    add_dotted_arrow(ax1, (V0, U0), (V0, U_GT), color="#c0392b", lw=4.2, mutation_scale=32,
    ...
    ax1.plot([V0, V0], [u_min, U0], color="#4e2103", ls=":", lw=1.6, zorder=4)
    ax1.plot([v_min, V0], [U0, U0], color="#4e2103", ls=":", lw=1.6, zorder=4)
    ax1.plot([v_min, V0], [U_GT, U_GT], color="#784212", ls=":", lw=1.6, zorder=4)
    ...
    # dA_X0
    add_curved_callout(ax1, fig, r"$\mathbf{\partial A_{X_0}}$", (2.15, float(U_adiabat(2.15, S0))), (2.35, 0.40), box_curve_adia, color=C_ADIA, fontsize=18.0, lw=1.5, rad=0.12, shrinkB=0)
    # dA_X1
    add_curved_callout(ax1, fig, r"$\mathbf{\partial A_{X_1}}$", (1.20, float(U_adiabat(1.20, S1))), (1.35, 3.65), box_curve_adia, color=C_ADIA, fontsize=18.0, lw=1.5, rad=-0.12, shrinkB=0)
    # I_T0
    ...
    add_curved_callout(ax1, fig, r"$\mathbf{X_0}$", (V0, U0), (0.98, 0.35), box_white, color="#4e2103", fontsize=17.5, lw=1.6, rad=-0.14, preferred_corner="tl", shrinkB=8)
    ...
    add_curved_callout(ax1, fig, r"$\mathbf{X_>}$", (V0, U_GT), (0.95, 3.45), box_gt, color="#784212", fontsize=17.5, lw=1.6, rad=-0.14, preferred_corner="bl", shrinkB=8)
    ...
    ax2.contourf(V_2D, U_2D, T_2D, levels=80, cmap="coolwarm", norm=norm, alpha=0.22, zorder=1)

    # Truncated adiabat dA_X on rho(A_X) = [1.15, 3.05] (so V0 = 0.82 notin rho(A_X))
    v_rho_min = 1.15
    mask_rho = (v_grid >= v_rho_min) & (u_adiaX_vals >= u_min) & (u_adiaX_vals <= u_max)
    ax2.plot(v_grid[mask_rho], u_adiaX_vals[mask_rho], color=C_ADIA, lw=3.8, ls="-", zorder=6)
    ax2.plot(v_grid[mask0], u_adia0_vals[mask0], color=C_ADIA, lw=2.4, ls="-", zorder=5)
    ax2.plot(v_grid[mask1], u_adia1_vals[mask1], color=C_ADIA, lw=2.4, ls="-", zorder=5)
    ax2.plot(v_grid[mask_iso], u_iso_vals[mask_iso], color=C_ISO, lw=3.6, ls=(0, (6, 3.5)), zorder=7)

    # Sub-step (a): Trapped vertical energy line ell over V_X0 (S < S(X))
    ax2.plot([V0, V0], [u_min, u_max], color="#784212", ls="--", lw=2.0, zorder=4)

    # Tag for trapped line ell
    box_ell_tag = dict(boxstyle="round,pad=0.18", facecolor="#fffcfc", edgecolor="#784212", lw=1.4, alpha=1.0)
    ax2.text(V0 - 0.04, u_max - 0.12,
             r"$\mathbf{\ell = \{(V_{X_0}, U)\}\ \subset\ \{S < S(X)\}}$" "\n" r"$\mathbf{(Trapped\ Line)}$",
             fontsize=12.5, fontweight="bold", color="#784212", ha="right", va="top", zorder=100, bbox=box_ell_tag)

    # Sub-step (b): Hot state X'_0 on ell (T5 yields T(X'_0) > T0, yet X'_0 << X)
    U0_PRIME = 2.40
    add_dotted_arrow(ax2, (V0, U0), (V0, U0_PRIME), color="#c0392b", lw=4.2, mutation_scale=32,
                     head_length=0.82, head_width=0.35, shrinkA=9.0, shrinkB=12.0, dot_spacing=2.0, zorder=9)
    box_arrow0 = dict(boxstyle="round,pad=0.18", facecolor="#fffcfc", edgecolor="#c0392b", lw=1.5, alpha=1.0)
    ax2.text(V0 + 0.05, 0.5 * (U0 + U0_PRIME),
             r"$\mathbf{Axiom\ T5:\ X^\prime_0 \in \Omega_>\ (X^\prime_0 \prec\prec X)}$",
             fontsize=13.0, fontweight="bold", color="#922b21", ha="left", va="center", zorder=100, bbox=box_arrow0)
    ...
    ax2.scatter([V0], [U0], color=C_PT_ISO, s=190, zorder=15, edgecolors="#4e2103", lw=2.2)
    ax2.scatter([V1], [U1], color=C_PT_ISO, s=190, zorder=15, edgecolors="#4e2103", lw=2.2)
    ax2.scatter([V0], [U0_PRIME], color="#e74c3c", s=210, zorder=15, edgecolors="#78281f", lw=2.2)
    ...
    add_region_label(ax2, fig, 2.30, 0.45, "minus", "#1b4f72", fontsize=33)
    add_region_label(ax2, fig, 1.45, 3.25, "plus", "#922b21", fontsize=33)

    # Projections on Case (b)
    ax2.plot([V0, V0], [u_min, U0], color="#4e2103", ls=":", lw=1.6, zorder=4)
    ax2.plot([v_min, V0], [U0, U0], color="#4e2103", ls=":", lw=1.6, zorder=4)
    ax2.plot([v_min, V0], [U0_PRIME, U0_PRIME], color="#922b21", ls=":", lw=1.6, zorder=4)
    ...
    # Bracket along x-axis for rho(A_X)
    ax2.annotate("", xy=(v_max, u_min), xytext=(v_rho_min, u_min),
                 arrowprops=dict(arrowstyle="-", color="#2c3e50", lw=5.0, capstyle="butt"))
    ax2.plot([v_rho_min, v_rho_min], [u_min - 0.05, u_min + 0.05], color="#2c3e50", lw=2.5, clip_on=False, zorder=10)

    ax2.set_xticks([V0, v_rho_min, V1])
    ax2.set_xticklabels([r"$\mathbf{V_{X_0}}$", r"$\mathbf{\inf \rho(A_X)}$", r"$\mathbf{V_{X_1}}$"], fontsize=14, fontweight="bold")
    ax2.set_yticks([U0, U1, U0_PRIME, U1_PRIME])
    ax2.set_yticklabels([r"$\mathbf{U(X_0)}$", r"$\mathbf{U(X_1)}$", r"$\mathbf{U(X^\prime_0)}$", r"$\mathbf{U(X^\prime_1)}$"], fontsize=14, fontweight="bold")

    # Callouts for Case (b)
    add_curved_callout(ax2, fig, r"$\mathbf{X_0}$", (V0, U0), (0.98, 0.35), box_white, color="#4e2103", fontsize=17.5, lw=1.6, rad=-0.14, preferred_corner="tl", shrinkB=8)
    ...
    add_curved_callout(ax2, fig, r"$\mathbf{X^\prime_0}$", (V0, U0_PRIME), (0.95, 2.75), box_prime0, color="#922b21", fontsize=17.5, lw=1.6, rad=-0.14, preferred_corner="bl", shrinkB=8)
    ...
    add_curved_callout(ax2, fig, r"$\mathbf{X_\uparrow \in \partial A_X \cap \Omega_>}$", (V_UP, U_UP), (0.95, 1.85), box_gt, color="#784212", fontsize=15.0, lw=1.6, rad=0.12, preferred_corner="tr", shrinkB=8)
    ...
    add_curved_callout(ax2, fig, r"$\mathbf{X^\prime \overset{A}{\sim} X}$", (V_PRIME, U_PRIME), (1.68, 1.25), box_cut, color=C_PT_CUT, fontsize=17.5, lw=1.8, rad=-0.14, preferred_corner="tl", shrinkB=10)
    ...
    add_curved_callout(ax2, fig, r"$\mathbf{\partial A_{X_0}}$", (2.15, float(U_adiabat(2.15, S0))), (2.35, 0.40), box_curve_adia, color=C_ADIA, fontsize=18.0, lw=1.5, rad=0.12, shrinkB=0)
    add_curved_callout(ax2, fig, r"$\mathbf{\partial A_{X_1}}$", (1.20, float(U_adiabat(1.20, S1))), (1.35, 3.65), box_curve_adia, color=C_ADIA, fontsize=18.0, lw=1.5, rad=-0.12, shrinkB=0)
...
generate_scenario1("scratch/test_scenario1_vu.png")
